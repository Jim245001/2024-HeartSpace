		...
		public string RenderHtml()
		{
			return $@"
				<div style='width: 200px; height: 133px; background-color: #d9d9d9; border-radius: 7px; padding: 7px; font-family: Arial, sans-serif; position: relative; font-size: 0.6rem;'>
					<div style='width: 40px; height: 40px; margin: 0 auto; background-color: #e7f3e8; border: 1px solid #c0c0c0; display: flex; align-items: center; justify-content: center;'>
						<img src='{ImageUrl}' alt='活動圖片' style='width: 90%; height: 90%; object-fit: cover;' />
					</div>
					<div style='margin-top: 7px; text-align: left;'>
						<p style='margin: 0;'><strong>標題：</strong>{Title}</p>
						<p style='margin: 0;'><strong>主辦：</strong>{Organizer}</p>
						<p style='margin: 0;'><strong>地點：</strong>{Location.Replace("\n", "<br />")}</p>
					</div>
